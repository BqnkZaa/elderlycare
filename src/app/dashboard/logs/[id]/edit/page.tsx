 ...
 * Edit Daily Log Page
 * 
 * Standalone form for editing an existing daily log entry.
 */
...
import { updateDailyLog, getDailyLogById } from '@/actions/daily-log.actions';
...
export default function EditDailyLogPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    ...
    useEffect(() => {
        async function fetchLogData() {
            ...
            if (result.success && result.data) {
                ...
                const vitals = log.vitals as DailyLogInput['vitals'];

                reset({
                    elderlyId: log.elderlyId,
                    date: new Date(log.date),
                    mood: log.mood as DailyLogInput['mood'],
                    mealIntake: log.mealIntake as DailyLogInput['mealIntake'],
                    sleepQuality: log.sleepQuality as DailyLogInput['sleepQuality'],
                    sleepHours: log.sleepHours ?? undefined,
                    medicationsTaken: log.medicationsTaken,
                    medicationNotes: log.medicationNotes ?? undefined,
                    activityNote: log.activityNote ?? undefined,
                    physicalCondition: log.physicalCondition ?? undefined,
                    behavioralNotes: log.behavioralNotes ?? undefined,
                    incidentsReported: log.incidentsReported ?? undefined,
                    recordedBy: log.recordedBy,
                    recordedByName: log.recordedByName ?? undefined,
    ...
    const onSubmit = async (data: unknown) => {
        ...
        try {
            const result = await updateDailyLog(id, formData);

            if (result.success) {
                router.push('/dashboard/logs');
            } else {
                setSubmitError(result.error || 'เกิดข้อผิดพลาดในการอัปเดต');
            }
        } catch (error) {
            console.error('Error updating log:', error);
            setSubmitError('เกิดข้อผิดพลาดในการอัปเดต');
        }
    ...
    if (notFound) {
        return (
            <div className="text-center py-24">
                ...
                <h2 className="text-xl font-semibold text-foreground mb-2">ไม่พบบันทึก</h2>
                <p className="text-muted-foreground mb-4">บันทึกที่คุณต้องการแก้ไขไม่มีอยู่ในระบบ</p>
